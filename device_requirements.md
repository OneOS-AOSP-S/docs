...
- The device **MUST** have complete hardware compatibility corresponding to the stock ROM, i.e. if IR blaster works on the stock ROM, it must work on OneOS AOSP. Only VoLTE is allowed to be ignored, so are NFC payment methods.

- The device **MUST NOT** use custom fingerprints (e.g. Pixels Build fingerprints). OneOS AOSP based on Android 12 is bypassing SafetyNet without any additional modifications on the device side.

- The device **MUST NOT** include any unused overlays and packages. This includes, but is not limited to, packages not being built, packages that don't work, obsolete packages, placebo 'tweaks' or any packages that will include unnecessary and/or unwanted features, as stated at the [Maintainers Code of Conduct](https://github.com/ONEOS-AOSP-S/docs/blob/main/maintainers_code_of_conduct.md).

- The device sources **MUST** be in accordance with, if applicable, all the items stated at our [Maintainers Code of Conduct](https://github.com/ONEOS-AOSP-S/docs/blob/main/maintainers_code_of_conduct.md) related to the device source code.
...
- If there are commits that are needed in repos other than the device-specific ones, they **MUST** be in accordance with our [Commit's Guidelines](https://github.com/ONEOS-AOSP-S/docs/blob/main/commits_guidelines.md).

- The device **MUST** have on it's sources commit complying with our [Commit's Guidelines](https://github.com/ONEOS-AOSP-S/docs/blob/main/commits_guidelines.md).
